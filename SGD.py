...
class SGD:
    ...
    def loss_function(self, phases):
        ...
        diff_sidelobe_ratio = (np.max(current_params["side_lobe_ratio"]) -
                               self.target_params["TargetSideLobeRatio"])  # 越小越好近0
        diff_peak_intensity = (np.min(current_params["intensity_peak"]) -
                               self.target_params["TargetPeakIntensity"])  # 绝对值越小越好，靠近2000
        diff_focal_offset = np.max(current_params["focal_offset"]) - self.target_params["TargetFocalOffset"]  # 焦移0.01以下
        # 计算焦深偏差越小越好
        ...
        total_loss = (d[0] * fwhm_loss +
                      d[1] * sidelobe_ratio_loss +
                      d[2] * peak_intensity_loss +
                      d[3] * focal_offset_loss +
                      d[4] * DOF_loss +
                      d[5] * intensity_sum_loss)
    ...
    def update_learning_rate(self, loss, learning_rate=0.01, decay_factor=0.1, min_learning_rate=1e-6, patience=0,
                             epoch=0,
                             best_loss=float('inf')):
        """
        更新学习率
