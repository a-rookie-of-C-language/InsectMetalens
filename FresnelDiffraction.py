...
class FresnelDiffraction:
    ...
    def compute_all(self, phases) -> dict:
        ...
        def calculate(i):
            ...
            # IPeak[i, :],
            # DOF[i] = self.fun_calculate_DOF(Zd, IPeak[i, :])
            DOF[i] = fun.CPSWFs_FWHM_calculation(IPeak[i, :], Zd / self.metalens.wavelength_center, self.metalens.nz)
            # :计算 intensity_sum 形状为 (len(phases))
            ...
            FWHM[i], side_lobe_ratio[i], intensity_peak[i] = self.EfieldParameters(Nz_intensity, x_coordinates,
                                                                                   self.metalens.n_n)
    ...
    def fun_calculate_DOF(self, Zd, IPeak):
        # NA = self.metalens.NA
