...
class MetalensOptimization:
    def __init__(self, opt: any = None):
        ...
        # 外径,单位为um
        self.outer_radius = 100*self.wavelength_center
        # 内径,单位为um
    ...
    def init_phases(self):
        ...
        # print(self.phases, end="\n")
        # delta_phi 的范围为 [-2pi, 2pi]
        # 使用gd来约束 delta_phi
        # df = pd.DataFrame({key: np.asnumpy(value) for key, value in self.phases})

        # file_name = datetime.now().strftime("result/data_%Y%m%d_%H%M/phases.csv")
        # df.to_csv(file_name, mode='a', header=True, index=False)
        print(self.phases)
        # todo phase
        if self.show:  # 这是原本的代码
    ...
    def phase_optimization(self):
        ...
        if self.method == "SGD":
            # 原代码max_iter为100000
            optimizer = SGD(learning_rate=0.1, max_iter=100, tol=1e-4, verbose=True, metalens=self)
            optimizer.fit()
            ...
            optimizer = NAG(learning_rate=0.8, max_iter=100, tol=1e-4, verbose=True, metalens=self)
            optimizer.fit()
        else:
